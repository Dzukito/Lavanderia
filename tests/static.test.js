...
assert.match(app, /pendingScheduleOrders[\s\S]*!\(order\.cycles \|\| \[\]\)\.length/, 'scheduled tickets should leave the pending ticket column after being dropped');
assert.match(app, /compactOrderTicketHtml[\s\S]*draggable="true"[\s\S]*data-drag-ticket[\s\S]*pending-ticket-main/, 'pending tickets should be draggable from their full card, not only nested buttons');
...
assert.match(app, /globalTurnHistoryHtml[\s\S]*Historial global[\s\S]*Sin máquina asignada/, 'turn history should be global in the schedule view for machine traceability');
assert.doesNotMatch(app, /cycleHistoryHtml\(order\)/, 'turn history should not be nested inside each order detail');
